...
class CODE128 : BaseCode {
	...
	private static readonly List<string> TABLE_A = new List<string> {
		" ", "!", "\"","#",
		"$", "%", "&", "'",
		"(", ")", "*", "+",
		",", "-", ".", "/",
	...
	private static readonly List<string> TABLE_B = new List<string> {
		" ", "!", "\"","#",
		"$", "%", "&", "'",
		"(", ")", "*", "+",
		",", "-", ".", "/",
	...
	private static readonly List<string> TABLE_C = new List<string> {
		"00", "01", "02", "03",
		"04", "05", "06", "07",
		"08", "09", "10", "11",
		"12", "13", "14", "15",
	...
	public override float Width {
		get {
			...
			var codeWidth = Pitch * len * 11;
			var spaceWidth = Pitch * QUIET_SIZE + BorderWidth;
			return codeWidth + spaceWidth * 2 + Pitch * 35;
		}
	}
	...
	public override void Draw() {
		...
		if (Regex.IsMatch(Value, "^[0-9]+$")) {
			table = TABLE_C;
			index = table.IndexOf("START_C");
			readLen = 2;
		} else {
			table = TABLE_B;
			index = table.IndexOf("START_B");
			readLen = 1;
		}
		...
		for (int pos = 0, weight = 1; pos < Value.Length; pos += readLen, weight++) {
			var chr = Value.Substring(pos, readLen);
			if (TABLE_B == table && !table.Contains(chr)) {
				chr = " ";
			}
			index = table.IndexOf(chr);
			sum += index * weight;
	...
	protected void DrawCode(int index) {
		for (int code = CODE[index], m = 0; code != 0; code >>= 4, m ^= 1) {
